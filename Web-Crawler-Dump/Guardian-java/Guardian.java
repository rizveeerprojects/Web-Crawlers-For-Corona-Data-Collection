...
class Fetcher {
    ...
    public Fetcher(String topicName) {
        ...
        this.fileName = "guardian_data_" + this.topic + ".csv";
        try {
    ...
    public ArrayList<String>linkValidityCheck(String link) {
        ...
        try{
            ...
            if(savenew.size()==6 && Integer.parseInt(savenew.get(2)) >=2019){
              flag=true;
            }
            else {
    ...
    public ArrayList<String> getFormattedData(String link) throws IOException{
      ...
      //debug: System.out.println("headline = " + sb.toString());
      extracted_data.add(sb.toString());

      //summary
      Elements summary = doc.select("div.content__standfirst > p");
      val = "";
      sb = new StringBuilder();
      sb.append(" ");
      ...
      //normal text
      Elements texts = doc.select("div.content__article-body > p");
      int cnt=0;
      sb = new StringBuilder();
      sb.append(" ");
      for (Element text : texts) {
    ...
    public int MonthNumber(String prefix){
      if(prefix.compareTo("jan") == 0) return 1;
      if(prefix.compareTo("feb") == 0) return 2;
      if(prefix.compareTo("mar") == 0) return 3;
      if(prefix.compareTo("apr") == 0) return 4;
      if(prefix.compareTo("may") == 0) return 5;
      if(prefix.compareTo("jun") == 0) return 6;
      if(prefix.compareTo("jul") == 0) return 7;
      if(prefix.compareTo("aug") == 0) return 8;
      if(prefix.compareTo("sep") == 0) return 9;
      if(prefix.compareTo("oct") == 0) return 10;
      if(prefix.compareTo("nov")  == 0)return 11;
      if(prefix.compareTo("dec") == 0) return 12;
      return 0;
    }
    ...
    public void getFetchedLinks(String mainPage) throws IOException {
        ...
        for (int i=0;i<articleLinks.size();i++) {
            ...
            //System.out.println(savenew.get(1).length());
            //System.out.println(savenew.get(0).length());
            //System.out.println(savenew.get(2));
            //System.out.println(savenew.get(3).length());
            date=metaData.get(i).get(4)+'/'+Integer.toString(MonthNumber(metaData.get(i).get(3)))+'/'+metaData.get(i).get(2); //date-month-year
            tags=metaData.get(i).get(1);
...
public class Guardian {
    ...
    public static String MonthPrefix(String month){
      int monthIdx = Integer.parseInt(month);
      ...
      prefix[10]="nov";
      prefix[11]="dec";
      return prefix[monthIdx-1];
    }

    public static boolean DataFetchInitiate(String day, String month, String year, Guardian newspaper){
        day = Padding(day,2);
        month = Padding(Integer.toString((Integer.parseInt(month)+1)),2);
        String baseUrl = "https://www.theguardian.com/theguardian/";
        String datePart=year+"/"+MonthPrefix(month)+"/"+day+'/';
        String page_link = baseUrl+datePart;
        try{
          newspaper.fetcher.getFetchedLinks(page_link);
          return true;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        Guardian newspaper = new Guardian();
